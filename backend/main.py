...
cookie_password = os.getenv("WORKOS_COOKIE_PASSWORD", "y0jN-wF1bIUoSwdKT6yWIHS5qLI4Kfq5TnqIANOxEXM=")
...
def create_cluster_in_pools(cluster_name: str, user: str = None, identity_file: str = None, password: str = None):
    """Create a new cluster in SSH node pools"""
    pools = load_ssh_node_pools()
    
    if cluster_name in pools:
        raise HTTPException(status_code=400, detail=f"Cluster '{cluster_name}' already exists")
...
def add_node_to_cluster(cluster_name: str, node: SSHNode):
    ...
    for existing_node in pools[cluster_name]["hosts"]:
        if existing_node.get("ip") == node.ip:
            raise HTTPException(status_code=400, detail=f"Node with IP '{node.ip}' already exists in cluster '{cluster_name}'")
...
async def add_node(cluster_name: str, add_node_request: AddNodeRequest, user = Depends(verify_auth)):
    """Add a node to an existing cluster"""
...
async def remove_node(cluster_name: str, node_ip: str, user = Depends(verify_auth)):
    ...
    if len(pools[cluster_name]["hosts"]) == original_length:
        raise HTTPException(status_code=404, detail=f"Node with IP '{node_ip}' not found in cluster '{cluster_name}'")
    
    save_ssh_node_pools(pools)
    
    return {"message": f"Node with IP '{node_ip}' removed from cluster '{cluster_name}' successfully"}
