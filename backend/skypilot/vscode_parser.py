...
def parse_vscode_tunnel_logs(logs: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        logger.error(f"Error parsing VSCode tunnel logs: {e}")
        return None, None
...
def is_vscode_tunnel_ready(logs: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Error checking VSCode tunnel readiness: {e}")
        return False
...
def get_vscode_tunnel_info(logs: str) -> dict:
    ...
    return {
        ...
        "status": "ready"
        if (auth_code is not None and tunnel_url is not None)
        else "loading",
    }
