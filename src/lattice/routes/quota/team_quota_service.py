...
def list_team_quotas(db: Session, organization_id: str) -> List[TeamQuota]:
    ...
    return (
        db.query(TeamQuota).filter(TeamQuota.organization_id == organization_id).all()
    )


def create_or_update_team_quota(
    db: Session, organization_id: str, team_id: str, monthly_credits_per_user: float
) -> TeamQuota:
    ...
    if team_quota:
        ...
        team_quota.monthly_credits_per_user = monthly_credits_per_user
...
def refresh_quota_periods_for_team_members(
    db: Session, organization_id: str, team_id: str
) -> None:
...
def team_quota_to_response(db: Session, team_quota: TeamQuota) -> TeamQuotaResponse:
    ...
    return TeamQuotaResponse(
        ...
        monthly_credits_per_user=team_quota.monthly_credits_per_user,
