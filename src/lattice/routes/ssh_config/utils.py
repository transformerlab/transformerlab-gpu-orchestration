...
def parse_ssh_config(config_content: str) -> dict:
    """
    Parse SSH config content into a JSON-serializable dictionary.
    Handles SSH config format with proper key-value parsing.
    """
    config_dict = {}
    ...
    for line in lines:
        ...
        # Handle comments - keep them for reference but mark as comment
        if line.startswith("#"):
            comment_key = f"comment_{len([k for k in config_dict.keys() if k.startswith('comment_')])}"
            config_dict[comment_key] = line
            continue
