...
class SkyPilotProvider(Provider):
    ...
    def _make_authenticated_request(
        self, method: str, endpoint: str, json_data: Optional[Dict[str, Any]] = None, timeout: Union[int, tuple] = 5, stream: bool = False
    ):
        """
    ...
    def launch_cluster(
        ...
        body = payloads.LaunchBody(
            ...
            is_launched_by_sky_serve_controller=config.provider_config.get("is_launched_by_sky_serve_controller", False),
    ...
    def stop_cluster(self, cluster_name: str) -> Dict[str, Any]:
        ...
        # Use SkyPilot's make_authenticated_request (matches SDK exactly)
        response = self._make_authenticated_request(
            'POST', '/down', json_data=body_json, timeout=5
        )
    ...
    def submit_job(self, cluster_name: str, job_config: JobConfig) -> Dict[str, Any]:
        ...
        # Use SkyPilot's make_authenticated_request (matches SDK exactly)
        response = self._make_authenticated_request(
            'POST', '/exec', json_data=body_json, timeout=5
        )
